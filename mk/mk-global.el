...
(π "<next> j r" #'skewer-repl)
(π "<next> j s" #'run-skewer)
...
(π "<next> p j" #'mk-python-run-dev-server)
...
(π "<next> v a" #'vimish-fold-avy)
(π "<next> v e" #'version)
(π "<next> v f" #'vimish-fold)
(π "<next> v r" #'split-window-right)
(π "<next> v v" #'vimish-fold-delete)
(π "<next> x i" #'mk-install)
(π "<next> x u" #'mk-uninstall)
(π "<next> x x" #'counsel-linux-app)
